    ...
    for (let index = 0; index < test_data.length; index++) {
        ...
        switch (condition) {
            case 'Login Yes':
                ...
                break;
            case 'Add Customer to Syspro Validations Yes':
                results = await addCustomerToSysProValidations(page, data1);
                testName = testCase;
                await returnResult(page, testName, results);
                break;
            case 'Exist Syspro Id to Add Customer Yes':
                results = await addCustomerToSyspro(page, data1, data2);
                testName = testCase;
                await returnResult(page, testName, results);
                break;
            case 'Verify View Permissions for Add Customer Yes':
                results = await addCustomerPermissions(page, 3);
                testName = testCase;
                await returnResult(page, testName, results);
                break;
            case 'Verify Edit Permissions for Add Customer Yes':
